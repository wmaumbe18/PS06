...
for (j in 1:n_sample) { #for each model
  ...
  out<-lm(y~x,current_sample_at_j) %>% 
    predict(test_set) 
  fitted[j]<-out #points on blue line
  out1<-smooth.spline(x=current_sample_at_j$x,y=current_sample_at_j$y, df=99) %>% 
    predict(test_set)
  fitted1[j]<-out1$y[[1]]#points on blue line
  }
 
actual<-rep(f(0.95),n_sample)
y<-rep(f(0.95),n_sample)+rnorm(n_sample,0,sigma)
biaslm<-mean(actual-fitted)
biassm<-mean(actual-fitted1)
varfittedlm<-var(fitted)
varfittedsm<-var(fitted1)
varerrorlm<-var(y)
varerrorsm<-var(y)
mselm<-mean((y-fitted)^2)
msesm<-mean((y-fitted1)^2)
sumlm<-biaslm^2+varerrorlm+varfittedlm
sumsm<-biassm^2+varerrorsm+varfittedsm
